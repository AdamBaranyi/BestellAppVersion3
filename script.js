...
function renderMenu() {
  for (let i = 0; i < menuData.length; i++) {
    const m = menuData[i];
    const grid = qs('[data-menu-grid="' + m.category + '"]');
...
function cartItemsHTML() {
  return cart.map(i => {
    ...
    return '<div class="cart__item">' +
      '<div>' + i.name + '</div><div style="text-align:right;">' + line + '</div>' +
      '<div class="qty">' +
      ...
      '<button class="qty-del" data-del="' + i.id + '">entfernen</button>' +
      '</div>' +
      '</div>';
  }).join("");
};
...
function renderAll() {
  renderCart({
    ...
    sub: "[data-subtotal]",
    tot: "[data-total]",
    mini: "[data-total-mini]",
    msg: "[data-order-message]"
  });
  renderCart({
    items: "[data-cart-items-mobile]",
    sub: "[data-subtotal-mobile]",
    tot: "[data-total-mobile]",
    mini: null,
